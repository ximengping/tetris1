...
int main( void )
{
	...
	draw_back(background1);
	next = rand() % 10;
        colour=rand()%6+1;
	show_next(next,&grade1);
	...
	while ( 1 ) {
		...
		if ( FD_ISSET(tfd, &rset)) {
			...
			timer_tetris(&t,background1,level1,&grade1);
		}
		if ( FD_ISSET(kfd, &rset) ) {
			tetris(&t,background1,&grade);
		}
                ...
                if ( FD_ISSET(nfd, &rset) ) {
                        ...
                        if ( read(nfd, &inf, sizeof inf) <= 0 ) {
                                ...
                                show_next(ntohl(inf.next),ntohl(inf.grade3));
                                next = ntohl(inf.next);
                                grade1= ntohl(inf.grade3);
                        }
                }
